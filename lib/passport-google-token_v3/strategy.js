 ...
 *
 * @param {Object} req
 * @options {Object}  optional  not used
 * @api protected
 */
  ...
  request(url, function (error, response, body) {
    if (!error && response.statusCode == 200) {
      var json = JSON.parse(body);
       console.log("json.aud=" + json.aud);
       console.log("self.clientID=" +JSON.stringify(self.clientID));

      if ( self.clientID.indexOf(json.aud) == -1){
          console.log("unauth");
          done('Invalid auth');
      }
      else{
